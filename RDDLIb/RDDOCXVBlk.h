...
namespace RDDLib
{
	namespace docxv
	{
		...
		public:
			...
			bool m_attached;
			static RDBmp^ m_def_bmp;
			static int m_cell_size;
			CRDVBlk(DOCXDoc^ doc, int pageno, double scale, int x, int y, int w, int h, double ph)
			{
				if (!m_def_bmp)
				{
					m_def_bmp = ref new RDBmp(2, 2);
					m_def_bmp->Reset(-1);
				}
				m_x = x;
				m_y = y;
			...
			inline void bk_render()
			{
				...
				m_page = DOCX_Document_getPage(m_doc, m_pageno);
				DOCX_PAGE page = m_page;
				PDF_DIB dib = Global_dibGet(NULL, m_w, m_h);
				DOCX_Page_renderPrepare(page, dib);
				if (m_status == -1)
				{
					Global_dibFree(dib);
					return;
				}
				DOCX_Page_render(page, dib, m_scale, m_x, m_y, mode_best);
			...
			inline bool ui_end(IVCallback^ canvas)
			{
				...
				if (m_status == 1 && m_page) DOCX_Page_renderCancel(m_page);
				m_status = -1;
				ui_cancel(canvas);
				return true;
			}
